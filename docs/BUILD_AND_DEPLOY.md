...
- `adafruit-nrfutil` (Python package, for DFU packaging — see §4)
...
1. **Double-tap the RST button** (or use `arduino-cli upload` once via serial to trigger the bootloader, then immediately hold the button). The board mounts as a **UF2BOOT** / **NRF52BOOT** drive.
