...
void rfSetAddrMulti(const uint8_t b4[4], uint8_t prefix)
{
	...
	for (int i = 0; i < 4; i++)
		b[i] = rfBitrev8(b4[i]);
	uint32_t base = ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) |
			((uint32_t)b[2] << 8) | b[3];
	NRF_RADIO->BASE0 = base;
	NRF_RADIO->BASE1 = base; // pipe 0 uses BASE0; pipes 1..7 use BASE1
	// 8 candidate prefixes: the learned one first, then a spread. De-dup isn't needed (a repeat just matches twice).
	uint8_t cand[8] = { prefix,
			    (uint8_t)(prefix ^ 0x01),
			    (uint8_t)(prefix + 1),
			    (uint8_t)(prefix - 1),
			    0x10,
	...
	uint8_t ap[8];
	for (int i = 0; i < 8; i++)
		ap[i] = g_prefixRaw ? cand[i] : rfBitrev8(cand[i]);
	NRF_RADIO->PREFIX0 = (uint32_t)ap[0] | ((uint32_t)ap[1] << 8) |
			     ((uint32_t)ap[2] << 16) | ((uint32_t)ap[3] << 24);
	NRF_RADIO->PREFIX1 = (uint32_t)ap[4] | ((uint32_t)ap[5] << 8) |
			     ((uint32_t)ap[6] << 16) | ((uint32_t)ap[7] << 24);
	NRF_RADIO->TXADDRESS = 0;
	NRF_RADIO->RXADDRESSES = 0xFF; // enable all 8 pipes
}
